...
object EnglishPipeline extends LanguagePipeline {
  ...
  def main(args: Array[String]) {
    ...
    println("\n*********************")
    println("Showing Organizations.")
    println("*********************")
    val organizationTaggedSentences = tokenizedSentences.map(organizationFinder.find(_))
    val organizations = tokenizedSentences.zip(organizationTaggedSentences).map {
      case (tokens, organizationTags) =>
        organizationTags.map(ptag => tokens.slice(ptag.getStart, ptag.getEnd).mkString(" "))
    }
    organizations.foreach { organizationsForSentence =>
      println("***")
      println(organizationsForSentence.mkString("\n"))
    }
...
object EnglishTools {
  ...
  lazy val organizationFinder =
    new NameFinderME(new TokenNameFinderModel(getEngResource("en-ner-organization.bin")))
